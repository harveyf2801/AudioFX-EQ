%% High Cut Band Class
% This function creates a high cut filter for an EQ.
%
...
classdef HighCutBand < FilterBand

    methods
        function obj = HighCutBand(centerFreq, q, fs)
            %%HIGH CUT BAND Constructor for the HighCutBand class.

            obj@FilterBand("High Cut", centerFreq, q, 0, fs); % default setting the dBgain to 0
        end

        function [b, a] = calculateCoefficients(obj)
            %%CALCULATE COEFFICIENTS Outputs IIR coeffs b and a for a standard high cut filter band.
